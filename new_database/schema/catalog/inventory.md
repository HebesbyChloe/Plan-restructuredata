...
| `location` | VARCHAR(100) | NOT NULL | Location code (e.g., 'VN', 'US', 'WAREHOUSE-001') |
...
- `idx_inventory_history_location` (on `location`)
...
- `idx_inventory_history_product_location` (on `product_id`, `location`) - For product-location queries
...
- **Location Codes**: Standardized location identifiers (e.g., 'VN', 'US', or specific warehouse codes)
- **Product & Stock Links**: 
  - `product_id` links directly to `product.id` for product information
  - `stock_id` links directly to `stock.id` for stock information
  - `recorded_quantity` should be populated from `stock.quantity_vn` or `stock.quantity_us` based on `location`
...
- **Data Source**: When creating audit record, get product info from `product` table and stock quantity from `stock` table based on location

**Example Usage:**

**Inventory Audit Record:**
- Product ID: 123, Stock ID: 456, Location: 'VN', Cycle: 1
- `product_id=123`, `stock_id=456`, `product_sku='PROD-001'`, `product_name='Sản phẩm A'`, `location='VN'`, `cycle_number=1`
- `recorded_quantity=100` (from `stock.quantity_vn`), `actual_quantity=95`, `quantity_discrepancy=-5` (shortage of 5 units)
...
2. Get stock info from `stock` table: `SELECT id, quantity_vn, quantity_us FROM stock WHERE product_sku = 'PROD-001'`
3. For location 'VN', use `stock.quantity_vn` as `recorded_quantity`
4. For location 'US', use `stock.quantity_us` as `recorded_quantity`
5. Insert audit record with `product_id`, `stock_id`, and denormalized `product_sku`, `product_name`

**Multiple Audit Cycles:**
- Same product, same location, different cycles
...
- Location-based queries help identify which warehouses have accuracy issues
...
- **Location Support**: Track audits by location (VN, US, or specific warehouse codes)
...
- **Data Source**: `recorded_quantity` is populated from `stock.quantity_vn` or `stock.quantity_us` based on location
...
```sql
SELECT ih.product_id, ih.product_sku, ih.product_name, ih.location, 
       COUNT(*) as audit_count,
       AVG(ih.quantity_discrepancy) as avg_discrepancy
FROM inventory_history ih
WHERE ih.status = 'completed'
GROUP BY ih.product_id, ih.product_sku, ih.product_name, ih.location
HAVING COUNT(*) > 1 AND AVG(ABS(ih.quantity_discrepancy)) > 5
ORDER BY avg_discrepancy DESC;
```

**Get latest audit for each product-location:**
```sql
SELECT DISTINCT ON (ih.product_id, ih.location) 
       ih.*, p.name as current_product_name, s.quantity_vn, s.quantity_us
FROM inventory_history ih
JOIN product p ON ih.product_id = p.id
JOIN stock s ON ih.stock_id = s.id
WHERE ih.status = 'completed'
ORDER BY ih.product_id, ih.location, ih.audit_date DESC;
```
...
SELECT ih.*, 
       ...
       CASE 
         WHEN ih.location = 'VN' THEN s.quantity_vn
         WHEN ih.location = 'US' THEN s.quantity_us
         ELSE NULL
