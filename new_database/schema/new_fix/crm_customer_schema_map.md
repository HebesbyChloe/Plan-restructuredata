...
└─────────────────────────────────────────────────────────────────┘
         ...
         └─── 1:N ────► project_projects (tenant_id)
...
| `lead_id` | INTEGER | FK → `crm_leads(id)` | 🔗 Current lead |
| `original_lead_id` | INTEGER | FK → `crm_leads(id)` | 🔗 Original lead |
...
| `tenant_id` | INTEGER | FK → `sys_tenants(id)` | 🔗 Tenant |
| `first_lead_id` | INTEGER | FK → `crm_leads(id)` | 🔗 First lead |
...
### 22. `project_projects`
**Purpose:** Project management. Links to SMS services and tasks.
...
**Indexes:**
- `idx_project_projects_tenant_id(tenant_id)` - Tenant filtering 📊
- `idx_project_projects_status(status)` - Status filtering 📊
- `idx_project_projects_dates(date_time_start, date_time_end)` - Date range queries 📊
...
| `linked_campaign_id` | INTEGER | FK → `mkt_campaigns(id)` | 🔗 Linked campaign |
| `linked_project_id` | INTEGER | FK → `project_projects(id)` | 🔗 Linked project |

**Foreign Keys:**
- `sender_account_id` → `SMS_sender_accounts(id)`
- `linked_campaign_id` → `mkt_campaigns(id)`
- `linked_project_id` → `project_projects(id)`
...
18. **`SMS_service_accounts` → `project_projects`** (Many-to-One)
