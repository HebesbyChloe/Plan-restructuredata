...
| `actor_id` | INTEGER | NULL, FK → `sys_users(id)` | 🔗 Staff member who performed action |
...
| `tenant_id` | BIGINT | NOT NULL, DEFAULT 1, FK → `sys_tenants(id)` | 🔗 Multi-tenant support |

**Foreign Keys:**
- `tenant_id` → `public.sys_tenants(id)` (Constraint: `logs_human_tenant_id_fkey`)
- `actor_id` → `public.sys_users(id)` (Constraint: `logs_human_actor_id_fkey`)
