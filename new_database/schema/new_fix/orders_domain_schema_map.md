...
│  • Links to: customers, addresses, tenants, stores, channels    │
│  • Unique: external_order_id (if applicable)                     │
│  • REQUIRED: store_id, channel_id                               │
└─────────────────────────────────────────────────────────────────┘
         │
         ├─── 1:N ────► order_items
         ├─── 1:1 ────► orders_meta
         ...
         ├─── N:1 ────► stores (store_id, REQUIRED)
         ├─── N:1 ────► channels_platform_pages (channel_id, REQUIRED)
...
└─────────────────────────────────────────────────────────────────┘
         ...
         ├─── N:1 ────► sys_users (sales_staff_id)
         └─── N:1 ────► sys_users (referrer_staff_id)
...
│    - Referenced by orders (created_by, updated_by)                 │
│    - Referenced by order_items (created_by, updated_by)             │
│    - Referenced by order_items_after_sales (created_by, updated_by) │
│    - Referenced by order_item_pre_orders (created_by, updated_by)   │
│    - Referenced by order_item_customization (created_by, updated_by) │
│    - Referenced by orders_meta (created_by, updated_by)              │
│    - Referenced by order_images (created_by)                        │
│    - Referenced by order_payments (created_by, updated_by)          │
│  • products               - Product catalog                        │
│    - Referenced by order_items (product_id)                        │
│  • order_item_statuses    - Order item status lookup                │
│    - Referenced by order_items (status)                             │
│  • pre_order_statuses     - Pre-order status lookup                 │
│    - Referenced by order_item_pre_orders (status)                   │
...
| `total` | NUMERIC | NULL, CHECK (total >= 0) | ✅ Total order amount (must be non-negative) |
...
| `tenant_id` | BIGINT | NOT NULL, FK → `sys_tenants(id)` | 🔗 Multi-tenant support |
| `store_id` | BIGINT | NOT NULL, FK → `stores(id)` | 🔗 Store where order was placed (REQUIRED) |
| `channel_id` | BIGINT | NOT NULL, FK → `channels_platform_pages(id)` | 🔗 Channel/platform where order originated (REQUIRED) |
| `customer_notes` | TEXT | NULL | Customer-provided notes |
| `created_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who created order |
...
- `store_id` → `stores(id)` (REQUIRED - every order must have a store)
- `channel_id` → `channels_platform_pages(id)` (REQUIRED - every order must have a channel)
- `created_by` → `sys_users(id)`
- `updated_by` → `sys_users(id)`

**Check Constraints:**
- `total >= 0` - Total must be non-negative

**Unique Constraints:**
- `UNIQUE(external_order_id, source_platform, tenant_id) WHERE external_order_id IS NOT NULL` - Unique external orders per platform per tenant
...
- `idx_orders_store_id(store_id)` - Store lookup 📊
- `idx_orders_channel_id(channel_id)` - Channel lookup 📊
...
- `idx_orders_tenant_status_date(tenant_id, status, date_created)` - Composite: common query pattern 📊
- `idx_orders_store_channel(store_id, channel_id)` - Composite: store and channel queries 📊
- `idx_orders_deleted_at(deleted_at) WHERE deleted_at IS NULL` - Partial index for active orders 📊
- `UNIQUE(external_order_id, source_platform, tenant_id) WHERE external_order_id IS NOT NULL` - Unique external orders 📊
...
- Channel-level order analytics
- Brand-level order aggregation (via store → brand relationship)
...
| `product_id` | BIGINT | NULL, FK → `product(id)` | 🔗 Product identifier (recommended FK) |
| `line_item_id` | VARCHAR | NULL | External line item ID |
| `status` | INTEGER | NULL, FK → `order_item_statuses(id)` | 🔗 Item status (recommended FK) |
| `quantity` | INTEGER | NOT NULL, DEFAULT 1, CHECK (quantity > 0) | ✅ Quantity ordered (must be positive) |
| `unit_price` | NUMERIC | NOT NULL, CHECK (unit_price >= 0) | ✅ Price per unit (must be non-negative) |
| `total_price` | NUMERIC | NOT NULL, GENERATED ALWAYS AS (quantity * unit_price) STORED | ✅ Total line price (calculated) |
| `created_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Creation timestamp |
| `updated_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Last update timestamp |
| `created_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who created item |
| `updated_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who last updated |
...
- `idx_order_items_order_status(order_id, status)` - Composite: order + status 📊
- `idx_order_items_deleted_at(deleted_at) WHERE deleted_at IS NULL` - Partial index for active items 📊
...
|--------|-----------|-------------|-------|
| `id` | BIGINT | PRIMARY KEY, GENERATED BY DEFAULT AS IDENTITY | Auto-incrementing primary key |
| `order_item_id` | BIGINT | NOT NULL, FK → `order_items(id)` | 🔗 Related order item |
| `original_order_item_id` | BIGINT | NULL, FK → `order_items(id)` | 🔗 Original order item (if different from order_item_id) |
| `case_type` | VARCHAR | NULL | Case type (e.g., 'return', 'repair', 'exchange') |
| `status` | INTEGER | NULL, FK → `service_statuses(id)` OR `return_statuses(id)` | 🔗 Status (recommended FK) |
| `rma_code` | VARCHAR | NULL, UNIQUE | 🔒 Return Merchandise Authorization code |
| `amount` | NUMERIC | NULL, CHECK (amount >= 0) | ✅ Refund/replacement amount (must be non-negative) |
...
| `created_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who created case |
| `updated_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who last updated |
| `created_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Creation timestamp |
| `updated_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Last update timestamp |
| `deleted_at` | TIMESTAMPTZ | NULL | ⏰ Soft delete timestamp |
...
- `status` → `service_statuses(id)` OR `return_statuses(id)` (recommended FK)
- `created_by` → `sys_users(id)`
- `updated_by` → `sys_users(id)`
...
**Unique Constraints:**
- `rma_code` (unique RMA codes)

**Indexes:**
- `idx_item_after_sales_order_item_id(order_item_id)` - Order item lookup 📊
- `idx_item_after_sales_status(status)` - Status filtering 📊
- `UNIQUE(rma_code)` - RMA code uniqueness 📊
- `idx_item_after_sales_deleted_at(deleted_at) WHERE deleted_at IS NULL` - Partial index for active cases 📊
...
|--------|-----------|-------------|-------|
| `id` | BIGINT | PRIMARY KEY, GENERATED BY DEFAULT AS IDENTITY | Auto-incrementing primary key |
| `order_item_id` | BIGINT | NOT NULL, FK → `order_items(id)` | 🔗 Parent order item |
| `status` | INTEGER | NULL, FK → `pre_order_statuses(id)` | 🔗 Pre-order status (recommended FK) |
...
| `created_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who created pre-order |
| `updated_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who last updated |
| `updated_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Last update timestamp |
| `created_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Creation timestamp |
| `deleted_at` | TIMESTAMPTZ | NULL | ⏰ Soft delete timestamp |
...
**Foreign Keys:**
- `order_item_id` → `order_items(id)` ON DELETE CASCADE
- `status` → `pre_order_statuses(id)` (recommended FK)
- `created_by` → `sys_users(id)`
- `updated_by` → `sys_users(id)`
...
- `idx_item_pre_orders_deleted_at(deleted_at) WHERE deleted_at IS NULL` - Partial index for active pre-orders 📊
...
|--------|-----------|-------------|-------|
| `id` | BIGINT | PRIMARY KEY, GENERATED BY DEFAULT AS IDENTITY | Auto-incrementing primary key |
| `order_item_id` | BIGINT | NOT NULL, UNIQUE, FK → `order_items(id)` | 🔗 🔒 Custom order item (1:1) |
| `actual_amount` | NUMERIC | NULL, CHECK (actual_amount >= 0) | ✅ Actual item amount (must be non-negative) |
| `balance_due` | NUMERIC | NULL, CHECK (balance_due >= 0) | ✅ Remaining balance (must be non-negative) |
...
| `created_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who created customization |
| `updated_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who last updated |
| `created_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Creation timestamp |
| `updated_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Last update timestamp |
| `deleted_at` | TIMESTAMPTZ | NULL | ⏰ Soft delete timestamp |
...
**Foreign Keys:**
- `order_item_id` → `order_items(id)` ON DELETE CASCADE (1:1 relationship)
...
**Check Constraints:**
- `actual_amount >= 0` - Actual amount must be non-negative
- `balance_due >= 0` - Balance due must be non-negative
...
- `idx_item_customization_deleted_at(deleted_at) WHERE deleted_at IS NULL` - Partial index for active customizations 📊
...
| `created_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who created metadata |
| `updated_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who last updated |
| `created_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Creation timestamp |
| `updated_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Last update timestamp |

**Foreign Keys:**
- `order_id` → `orders(id)` ON DELETE CASCADE (1:1 relationship)
- `created_by` → `sys_users(id)`
- `updated_by` → `sys_users(id)`
...
| `source_pancake_id` | BIGINT | NULL, FK → `channels_platform_pages(id)` | 🔗 Source platform page |
| `sales_staff_id` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 Staff who closed the order |
| `referrer_staff_id` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 Staff who referred/introduced |
...
**Foreign Keys:**
- `id` → `orders(id)` ON DELETE CASCADE (1:1 relationship, PRIMARY KEY)
- `source_pancake_id` → `channels_platform_pages(id)` (recommended FK)
- `sales_staff_id` → `sys_users(id)` (recommended FK)
- `referrer_staff_id` → `sys_users(id)` (recommended FK)
...
| `created_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who uploaded image |
| `created_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Creation timestamp |
| `deleted_at` | TIMESTAMPTZ | NULL | ⏰ Soft delete timestamp |
...
| `created_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who created payment |
| `updated_by` | BIGINT | NULL, FK → `sys_users(id)` | 🔗 User who last updated |
| `created_at` | TIMESTAMPTZ | NOT NULL, DEFAULT now() | ⏰ Creation timestamp |
...
13. **`orders` → `stores`** (Many-to-One, REQUIRED)
    - Many orders belong to one store
    - `orders.store_id` → `stores.id`
    - **REQUIRED:** Every order must have a store
    - Store belongs to a brand, enabling brand-level order aggregation

14. **`orders` → `channels_platform_pages`** (Many-to-One, REQUIRED)
    - Many orders belong to one channel/platform page
    - `orders.channel_id` → `channels_platform_pages.id`
    - **REQUIRED:** Every order must have a channel
    - Channel belongs to a brand (via channels_platforms), enabling brand-level channel analytics
...
17. **`order_meta_crm` → `channels_platform_pages`** (Many-to-One)
    - Source page: `order_meta_crm.source_pancake_id` → `channels_platform_pages.id`

18. **`order_meta_crm` → `sys_users`** (Many-to-One, Two Relationships)
    - Sales staff: `order_meta_crm.sales_staff_id` → `sys_users.id`
    - Referrer staff: `order_meta_crm.referrer_staff_id` → `sys_users.id`
...
- `idx_order_images_order_id(order_id)` - Order lookup 📊
- `idx_order_images_image_type(image_type)` - Type filtering 📊
- `idx_order_images_deleted_at(deleted_at) WHERE deleted_at IS NULL` - Partial index for active images 📊
...
- `idx_order_payment_order_paid_date(order_id, paid_date DESC)` - Composite: order + paid date 📊
- `uq_order_payment_transaction_id_nonempty(transaction_id) WHERE transaction_id <> ''` - Unique transaction IDs 📊
- `idx_order_payment_deleted_at(deleted_at) WHERE deleted_at IS NULL` - Partial index for active payments 📊
...
3. **Unique Constraints:**
   - `UNIQUE(external_order_id, source_platform, tenant_id) WHERE external_order_id IS NOT NULL` - Unique external orders per platform per tenant
   - `order_items_after_sales.rma_code` - Unique RMA codes
   - `order_items.total_price` - Generated column (calculated from quantity × unit_price)
...
- **Stores:** `orders.store_id` → `stores.id` (REQUIRED)
  - Every order must be associated with a store
  - Store belongs to a brand, enabling brand-level order aggregation
  - Store belongs to a tenant, maintaining multi-tenancy
- **Channels:** `orders.channel_id` → `channels_platform_pages.id` (REQUIRED)
  - Every order must be associated with a channel/platform page
  - Channel belongs to a brand (via channels_platforms), enabling brand-level channel analytics
  - Channel belongs to a tenant, maintaining multi-tenancy

### CRM Integration
- **Source Tracking:** `order_meta_crm.source_pancake_id` → `channels_platform_pages.id` (platform pages)
- **Staff Tracking:** `order_meta_crm.sales_staff_id` → `sys_users.id` (sales staff)
- **Referrer Tracking:** `order_meta_crm.referrer_staff_id` → `sys_users.id` (referrer staff)
...
- **External Orders:** `external_order_id` + `source_platform` + `tenant_id` composite unique constraint enables multi-platform order aggregation
...
- **Store & Channel Requirements:** Every order must have both `store_id` and `channel_id` (NOT NULL constraints)
- **Brand-Level Analytics:** Orders can be aggregated by brand via store → brand relationship and channel → brand relationship
- **Tenant Isolation:** All queries must filter by `tenant_id` for proper multi-tenant data isolation
- **Soft Delete:** All tables support soft delete via `deleted_at` timestamp for data retention
- **Audit Trail:** All tables include `created_by`, `updated_by` for audit tracking
- **Data Integrity:** CHECK constraints ensure business rules (positive quantities, non-negative amounts)
- **Generated Columns:** `order_items.total_price` is calculated automatically from quantity × unit_price
- **Status Management:** All status fields reference lookup tables via FK for data consistency
