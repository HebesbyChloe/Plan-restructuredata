...
CREATE TABLE IF NOT EXISTS orders (
    ...
    CONSTRAINT uq_orders_external_order UNIQUE (external_order_id, source_platform, tenant_id) 
...
-- Note: product_id FK will be added after product table is created
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS order_items_after_sales (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    ...
    CONSTRAINT chk_order_items_after_sales_amount CHECK (amount >= 0)
);

COMMENT ON TABLE order_items_after_sales IS 'After-sales service cases including returns, repairs, and exchanges at item level.';
COMMENT ON COLUMN order_items_after_sales.order_item_id IS 'Related order item';
COMMENT ON COLUMN order_items_after_sales.original_order_item_id IS 'Original order item (if different)';
COMMENT ON COLUMN order_items_after_sales.rma_code IS 'Return Merchandise Authorization code';

-- Foreign Keys
ALTER TABLE order_items_after_sales 
    ADD CONSTRAINT fk_order_items_after_sales_order_item_id 
    FOREIGN KEY (order_item_id) REFERENCES order_items(id) ON DELETE CASCADE;
    
ALTER TABLE order_items_after_sales 
    ADD CONSTRAINT fk_order_items_after_sales_original_order_item_id 
    FOREIGN KEY (original_order_item_id) REFERENCES order_items(id);
    
ALTER TABLE order_items_after_sales 
    ADD CONSTRAINT fk_order_items_after_sales_created_by 
    FOREIGN KEY (created_by) REFERENCES sys_users(id);
    
ALTER TABLE order_items_after_sales 
    ADD CONSTRAINT fk_order_items_after_sales_updated_by 
    FOREIGN KEY (updated_by) REFERENCES sys_users(id);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_item_after_sales_order_item_id ON order_items_after_sales(order_item_id);
CREATE INDEX IF NOT EXISTS idx_item_after_sales_status ON order_items_after_sales(status);
CREATE INDEX IF NOT EXISTS idx_item_after_sales_deleted_at ON order_items_after_sales(deleted_at) WHERE deleted_at IS NULL;

-- ----------------------------------------------------------------------------
-- 4. order_item_pre_orders
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS order_item_pre_orders (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    order_item_id BIGINT NOT NULL,
    status INTEGER NULL,
...
COMMENT ON TABLE order_item_pre_orders IS 'Pre-order records tied to order items - tracks items that are ordered but not yet available.';
COMMENT ON COLUMN order_item_pre_orders.order_item_id IS 'Parent order item';
COMMENT ON COLUMN order_item_pre_orders.hold_until IS 'Date to hold order until';

-- Foreign Keys
ALTER TABLE order_item_pre_orders 
    ADD CONSTRAINT fk_order_item_pre_orders_order_item_id 
    FOREIGN KEY (order_item_id) REFERENCES order_items(id) ON DELETE CASCADE;
    
ALTER TABLE order_item_pre_orders 
    ADD CONSTRAINT fk_order_item_pre_orders_created_by 
    FOREIGN KEY (created_by) REFERENCES sys_users(id);
    
ALTER TABLE order_item_pre_orders 
    ADD CONSTRAINT fk_order_item_pre_orders_updated_by 
    FOREIGN KEY (updated_by) REFERENCES sys_users(id);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_item_pre_orders_order_item_id ON order_item_pre_orders(order_item_id);
CREATE INDEX IF NOT EXISTS idx_item_pre_orders_hold_until ON order_item_pre_orders(hold_until);
CREATE INDEX IF NOT EXISTS idx_item_pre_orders_deleted_at ON order_item_pre_orders(deleted_at) WHERE deleted_at IS NULL;
...
CREATE TABLE IF NOT EXISTS order_item_customization (
    ...
    CONSTRAINT chk_order_item_customization_actual_amount CHECK (actual_amount >= 0),
    CONSTRAINT chk_order_item_customization_balance_due CHECK (balance_due >= 0)
);

COMMENT ON TABLE order_item_customization IS 'Custom item-level workflow and billing for bespoke/custom-made jobs.';
COMMENT ON COLUMN order_item_customization.order_item_id IS 'Custom order item (1:1)';
COMMENT ON COLUMN order_item_customization.actual_amount IS 'Actual item amount (must be non-negative)';
COMMENT ON COLUMN order_item_customization.balance_due IS 'Remaining balance (must be non-negative)';

-- Foreign Keys
ALTER TABLE order_item_customization 
    ADD CONSTRAINT fk_order_item_customization_order_item_id 
    FOREIGN KEY (order_item_id) REFERENCES order_items(id) ON DELETE CASCADE;
    
ALTER TABLE order_item_customization 
    ADD CONSTRAINT fk_order_item_customization_created_by 
    FOREIGN KEY (created_by) REFERENCES sys_users(id);
    
ALTER TABLE order_item_customization 
    ADD CONSTRAINT fk_order_item_customization_updated_by 
    FOREIGN KEY (updated_by) REFERENCES sys_users(id);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_item_customization_statuses ON order_item_customization(payment_status, design_status, material_status, completion_status);
CREATE INDEX IF NOT EXISTS idx_item_customization_deleted_at ON order_item_customization(deleted_at) WHERE deleted_at IS NULL;
...
COMMENT ON COLUMN order_meta_crm.source_pancake_id IS 'Source platform page';
...
ALTER TABLE order_meta_crm 
    ADD CONSTRAINT fk_order_meta_crm_source_pancake_id 
    FOREIGN KEY (source_pancake_id) REFERENCES channels_platform_pages(id);
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_order_meta_crm_source_pancake_id ON order_meta_crm(source_pancake_id);
...
CREATE TRIGGER trg_order_items_after_sales_updated_at
    BEFORE UPDATE ON order_items_after_sales
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_order_item_pre_orders_updated_at
    BEFORE UPDATE ON order_item_pre_orders
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trg_order_item_customization_updated_at
    BEFORE UPDATE ON order_item_customization
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();
...
-- ALTER TABLE order_items ADD CONSTRAINT fk_order_items_product_id FOREIGN KEY (product_id) REFERENCES product(id);
