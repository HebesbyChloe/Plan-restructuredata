...
ALTER TABLE product 
    ...
    CHECK (product_type IN ('standard', 'custom', 'variant', 'set', 'jewelry', 'diamond', 'gemstone'));

ALTER TABLE product 
    ADD CONSTRAINT chk_product_status 
    CHECK (status IN ('draft', 'publish', 'updated', 'do_not_import'));
...
ALTER TABLE product_category 
    ADD CONSTRAINT uq_product_category_product_category 
...
ALTER TABLE product_tag 
    ADD CONSTRAINT uq_product_tag_product_tag 
...
-- Step 3: UNIQUE constraints
ALTER TABLE warehouse 
    ADD CONSTRAINT uq_warehouse_code 
    UNIQUE (code);
...
ALTER TABLE stock 
    ADD CONSTRAINT chk_stock_outbound 
    CHECK (outbound >= 0);

ALTER TABLE stock 
    ADD CONSTRAINT chk_stock_inbound 
    CHECK (inbound >= 0);

-- Step 3: UNIQUE constraints
ALTER TABLE stock 
    ADD CONSTRAINT uq_stock_product_sku_warehouse 
...
ALTER TABLE product_attribute 
    ADD CONSTRAINT uq_product_attribute_name 
    UNIQUE (name);
...
ALTER TABLE product_attribute_value 
    ADD CONSTRAINT uq_product_attribute_value_product_attribute 
...
ALTER TABLE product_set_item 
    ADD CONSTRAINT chk_product_set_item_quantity 
    CHECK (quantity > 0);

-- Step 3: UNIQUE constraints
ALTER TABLE product_set_item 
    ADD CONSTRAINT uq_product_set_item_set_item 
...
ALTER TABLE product_variant 
    ADD CONSTRAINT uq_product_variant_parent_attribute_value 
    UNIQUE (parent_product_id, variant_attribute, variant_value);

ALTER TABLE product_variant 
    ADD CONSTRAINT uq_product_variant_variant_product 
...
ALTER TABLE product_image 
    ADD CONSTRAINT uq_product_image_product_id 
    UNIQUE (product_id);
...
-- Step 2: CHECK constraints
ALTER TABLE diamond 
    ADD CONSTRAINT chk_diamond_carat 
    CHECK (carat > 0);

-- Step 3: UNIQUE constraints
ALTER TABLE diamond 
    ADD CONSTRAINT uq_diamond_product_id 
    UNIQUE (product_id);

ALTER TABLE diamond 
    ADD CONSTRAINT uq_diamond_item_id 
    UNIQUE (item_id) WHERE item_id IS NOT NULL;

ALTER TABLE diamond 
    ADD CONSTRAINT uq_diamond_certificate_number 
    UNIQUE (certificate_number) WHERE certificate_number IS NOT NULL;
...
ALTER TABLE gemstone 
    ADD CONSTRAINT chk_gemstone_carat 
    CHECK (carat > 0);

-- Step 3: UNIQUE constraints
ALTER TABLE gemstone 
    ADD CONSTRAINT uq_gemstone_product_id 
    UNIQUE (product_id);

ALTER TABLE gemstone 
    ADD CONSTRAINT uq_gemstone_item_id 
    UNIQUE (item_id) WHERE item_id IS NOT NULL;

ALTER TABLE gemstone 
    ADD CONSTRAINT uq_gemstone_certificate_number 
    UNIQUE (certificate_number) WHERE certificate_number IS NOT NULL;
...
ALTER TABLE material 
    ADD CONSTRAINT chk_material_stock_vn 
    CHECK (stock_vn >= 0);

ALTER TABLE material 
    ADD CONSTRAINT chk_material_stock_us 
    CHECK (stock_us >= 0);

-- Step 3: UNIQUE constraints
ALTER TABLE material 
    ADD CONSTRAINT uq_material_sku 
    UNIQUE (sku);
...
ALTER TABLE material_attribute 
    ADD CONSTRAINT fk_material_attribute_material_id 
...
ALTER TABLE material_attribute 
    ADD CONSTRAINT fk_material_attribute_attribute_id 
    FOREIGN KEY (attribute_id) REFERENCES product_attribute(id) ON DELETE SET NULL;
...
COMMENT ON COLUMN material_attribute.material_id IS 'Material ID (FK to material table)';
COMMENT ON COLUMN material_attribute.tenant_id IS 'Tenant identifier';
...
-- Step 6: Indexes
CREATE INDEX IF NOT EXISTS idx_material_attribute_material_id ON material_attribute(material_id);
...
ALTER TABLE product_review 
    ADD CONSTRAINT chk_product_review_rating 
    CHECK (rating >= 1 AND rating <= 5);

ALTER TABLE product_review 
    ADD CONSTRAINT chk_product_review_helpful_count 
    CHECK (helpful_count >= 0);

ALTER TABLE product_review 
    ADD CONSTRAINT chk_product_review_status 
...
-- Thứ tự migrate (đã có category)
-- Level 1: Master tables (không phụ thuộc product)
-- ✅ category — đã migrate
-- product_attribute — master table, không có FK phụ thuộc
-- warehouse — cần sys_users (từ 001)
-- material — cần sys_users (từ 001)
-- Level 2: Core product table
-- product — cần sys_users (từ 001), promotion_id có thể NULL/0
-- Level 3: Tables phụ thuộc product
-- product_category — cần product + category (đã có)
-- product_tag — cần product
-- product_attribute_value — cần product + product_attribute
-- product_image — cần product + sys_users
-- diamond — cần product (one-to-one)
-- gemstone — cần product (one-to-one)
-- product_set_item — cần product (self-referencing)
-- product_variant — cần product (self-referencing)
-- material_product — cần product + material
-- stock — cần warehouse (soft FK với product.sku)
-- material_attribute — cần material + sys_tenants
-- product_review — cần product + crm_personal_keys + sys_users
