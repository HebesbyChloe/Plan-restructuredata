...
CREATE TABLE IF NOT EXISTS stores (
    ...
    CONSTRAINT chk_stores_status CHECK (status IN ('active', 'inactive', 'closed')),
    CONSTRAINT chk_stores_closing_date CHECK (closing_date IS NULL OR opening_date IS NULL OR closing_date >= opening_date),
    CONSTRAINT uq_stores_tenant_code UNIQUE (tenant_id, code),
    CONSTRAINT uq_stores_tenant_name UNIQUE (tenant_id, name)
);

COMMENT ON TABLE stores IS 'Physical or virtual store locations belonging to a tenant. One company (tenant) can have many stores, but each store belongs to exactly one company.';
COMMENT ON COLUMN stores.id IS 'Auto-incrementing primary key';
COMMENT ON COLUMN stores.tenant_id IS 'Tenant owner (CASCADE on tenant delete)';
COMMENT ON COLUMN stores.code IS 'Machine-readable store code (e.g., ''STORE-001'', ''HQ'')';
COMMENT ON COLUMN stores.name IS 'Human-readable store name (e.g., ''Main Store'', ''Downtown Branch'')';
COMMENT ON COLUMN stores.status IS 'Status: ''active'', ''inactive'', ''closed''';
COMMENT ON COLUMN stores.is_headquarters IS 'Whether this is the company headquarters';
COMMENT ON COLUMN stores.manager_user_id IS 'Store manager';

-- Foreign Keys
ALTER TABLE stores 
    ADD CONSTRAINT fk_stores_tenant_id 
    FOREIGN KEY (tenant_id) REFERENCES sys_tenants(id) ON DELETE CASCADE;
    
ALTER TABLE stores 
    ADD CONSTRAINT fk_stores_manager_user_id 
    FOREIGN KEY (manager_user_id) REFERENCES sys_users(id);
...
ALTER TABLE stores 
    ADD CONSTRAINT fk_stores_updated_by 
...
CREATE INDEX IF NOT EXISTS idx_stores_manager ON stores(manager_user_id);
CREATE INDEX IF NOT EXISTS idx_stores_is_headquarters ON stores(tenant_id, is_headquarters) WHERE is_headquarters = true;
...
CREATE INDEX IF NOT EXISTS idx_stores_opening_date ON stores(opening_date);
