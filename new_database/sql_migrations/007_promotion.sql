...
-- ============================================================================
-- Step 1: CREATE TABLE (only columns and PRIMARY KEY)
-- ============================================================================
...
-- promo
ALTER TABLE promo 
    ADD CONSTRAINT chk_promo_type 
    CHECK (promo_type IN ('discount', 'bogo', 'bundle', 'tiered', 'free_shipping'));

ALTER TABLE promo 
    ADD CONSTRAINT chk_promo_status 
    CHECK (status IN ('draft', 'active', 'paused', 'expired', 'archived'));

-- promo_discount_rules
ALTER TABLE promo_discount_rules 
    ADD CONSTRAINT chk_promo_discount_rules_discount_type 
    CHECK (discount_type IN ('percentage', 'fixed_amount', 'fixed_price', 'free_shipping'));

ALTER TABLE promo_discount_rules 
    ADD CONSTRAINT chk_promo_discount_rules_apply_to 
    CHECK (apply_to IN ('order', 'line_item', 'shipping'));

ALTER TABLE promo_discount_rules 
    ADD CONSTRAINT chk_promo_discount_rules_allocation_method 
    CHECK (allocation_method IS NULL OR allocation_method IN ('across', 'each'));

-- promo_bogo_rules
ALTER TABLE promo_bogo_rules 
    ADD CONSTRAINT chk_promo_bogo_rules_buy_type 
    CHECK (buy_type IN ('quantity', 'amount', 'specific_products'));

ALTER TABLE promo_bogo_rules 
    ADD CONSTRAINT chk_promo_bogo_rules_get_type 
    CHECK (get_type IN ('same_product', 'specific_products', 'cheapest', 'any'));

-- promo_bundle_rules
ALTER TABLE promo_bundle_rules 
    ADD CONSTRAINT chk_promo_bundle_rules_bundle_type 
    CHECK (bundle_type IN ('quantity', 'amount', 'specific_products'));

ALTER TABLE promo_bundle_rules 
    ADD CONSTRAINT chk_promo_bundle_rules_discount_type 
    CHECK (discount_type IN ('percentage', 'fixed_amount', 'fixed_price'));

-- promo_tiered_rules
ALTER TABLE promo_tiered_rules 
    ADD CONSTRAINT chk_promo_tiered_rules_threshold_type 
    CHECK (threshold_type IN ('quantity', 'amount'));

ALTER TABLE promo_tiered_rules 
    ADD CONSTRAINT chk_promo_tiered_rules_discount_type 
    CHECK (discount_type IN ('percentage', 'fixed_amount'));

-- promo_product_price_overrides
ALTER TABLE promo_product_price_overrides 
    ADD CONSTRAINT chk_promo_price_overrides_type 
    CHECK (price_override_type IN ('sale_price', 'retail_price', 'both'));

-- promo_codes
ALTER TABLE promo_codes 
    ADD CONSTRAINT chk_promo_codes_status 
    CHECK (status IN ('active', 'used', 'expired', 'revoked'));

-- promo_eligible_products
ALTER TABLE promo_eligible_products 
    ADD CONSTRAINT chk_promo_eligible_products_inclusion_type 
    CHECK (inclusion_type IN ('include', 'exclude'));

ALTER TABLE promo_eligible_products 
    ADD CONSTRAINT chk_promo_eligible_products_context 
    CHECK (context IS NULL OR context IN ('buy_side', 'get_side', 'both'));
...
-- promo
ALTER TABLE promo 
    ADD CONSTRAINT uq_promo_promo_code 
    UNIQUE (promo_code);

-- promo_campaigns
ALTER TABLE promo_campaigns 
    ADD CONSTRAINT uq_promo_campaigns_promo_campaign 
    UNIQUE (promo_id, mkt_campaign_id);

-- promo_discount_rules
ALTER TABLE promo_discount_rules 
    ADD CONSTRAINT uq_promo_discount_rules_promo_id 
    UNIQUE (promo_id);

-- promo_bogo_rules
ALTER TABLE promo_bogo_rules 
    ADD CONSTRAINT uq_promo_bogo_rules_promo_id 
    UNIQUE (promo_id);

-- promo_bundle_rules
ALTER TABLE promo_bundle_rules 
    ADD CONSTRAINT uq_promo_bundle_rules_promo_id 
    UNIQUE (promo_id);

-- promo_codes
ALTER TABLE promo_codes 
    ADD CONSTRAINT uq_promo_codes_code 
    UNIQUE (code);
