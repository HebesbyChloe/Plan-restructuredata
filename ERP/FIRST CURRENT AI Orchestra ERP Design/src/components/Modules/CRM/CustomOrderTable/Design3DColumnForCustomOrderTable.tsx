...
import { DESIGN_3D_STATUS, getCustomizeOrderExtraByOrderNumber } from "../../../../sampledata/customizeOrderExtraData";
import { calculateDaysBetween, formatDaysDisplay } from "../../../../utils/modules/crm";
import { InfoBadge } from "../../../ui/info-badge";

const DESIGN_3D_STATUSES = Object.values(DESIGN_3D_STATUS);

export function Design3DColumnForCustomOrderTable({ order, onOrderInfoClick }: CustomOrderColumnProps) {
  // Get 3D design status from extra data mapped by order number
  const extraData = getCustomizeOrderExtraByOrderNumber(order.orderNumber);
  const design3DStatus = extraData?.design3DStatus || order.customizeDetails?.design3DStatus || "Not Started";
  const updatedDate = extraData?.design3DUpdatedDate || "—";
  const updatedBy = extraData?.design3DUpdatedBy || "—";
    ...
    >
      <div className="flex flex-col gap-2.5">
        ...
        <div className="min-h-[26px] flex items-center">
          {updatedDate !== "—" && (
            <div className={`px-2 py-0.5 rounded text-xs ${daysTaken > 2 ? 'bg-red-500/10 text-red-600 dark:bg-red-500/20 dark:text-red-400' : 'bg-muted text-muted-foreground'}`}>
